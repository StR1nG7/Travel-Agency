...
const Tour: React.FC<ITour> = (
	...
	for (let i = 0; i < hotels.length; i++) {
		...
		for (let j = 1; j <= +hotels[i]; j++) {
			stars[i].push(<i className="mdi mdi-star" key={j}></i>);
		}
			...
			<div className="col-12 col-sm-6 col-lg-4">
				<STourWrapper>
					<div style={{position:'relative'}}>
						<Link to={`/tours/${id}`}>
						<ImageWithWebp srcWebp={`/public/img/${replacedTitle}.webp`} fallback={`/public/img/${replacedTitle}.jpg`} alt="Tour" style={{width:'100%'}}/>
						</Link>
						<STourPrice>
							<h5 style={{marginBottom:0,fontWeight:500,color:'#fff'}}>${price}</h5>/tour
						</STourPrice>
					</div>
					<STourCaption>
						<STourTitleWrapper>
							...
							<div style={{fontSize:'0.9375em'}}>From: {from.join(', ')}</div>
							<div style={{fontSize:'0.9375em'}}>{persons} persons</div>
						</STourTitleWrapper>
						<STourDaysWrapper>
							<STourDays><span style={{marginRight:10}}>{period} Days</span></STourDays>
							<SRating>
								{starsGroups}
							</SRating>
						</STourDaysWrapper>
