...
const TourFilter: React.FC = () => {
	...
	const minPrice = useSelector((state: TRootState) => (state.toursPage as IToursPageReducer).minPrice);
	const maxPrice = useSelector((state: TRootState) => (state.toursPage as IToursPageReducer).maxPrice);
										...
										<STourFilterPrice>
											<STourFilterTitle>
												...
												<span style={{position:'relative',left: -22}}>
													Max. price, $:<span style={{position:'absolute',width: 43,left: 'calc(100% + 3px)'}}>{values.price ? values.price : maxPrice}</span>
												</span>
												<span>{maxPrice}</span>
											</STourFilterTitle>
											<Field type="range" name="price" min={minPrice} max={maxPrice} step="100"
														 value={values.price ? values.price : maxPrice}
														 onChange={ (e: React.KeyboardEvent<HTMLInputElement>) => {
															 handleChange(e); // Formik handleChange
															 const _value = e.currentTarget.value;
															 if (+_value) {
																 dispatch(setCurrentFilter({price: _value}));
																 dispatch(setCurrentPage(1));
																 dispatch(getToursThunkCreator({currentFilters: {...currentFilters, price: _value}}));
															 }
														 } }
											/>
										</STourFilterPrice>
